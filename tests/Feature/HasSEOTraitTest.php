...
beforeEach(function () {
    ...
    config(['seo.features.auto_analyze' => true]);
...
describe('HasSEO Trait', function () {
    ...
    it('dispatches analysis job on content change', function () {
        config(['seo.features.auto_analyze' => true]);
        ...
        Queue::assertPushed(AnalyzeContentJob::class); // From creation

        // Clear queue
        Queue::fake();

        // Update content field
        $post->update(['content' => 'Updated content that should trigger analysis']);

        Queue::assertPushed(AnalyzeContentJob::class, function ($job) use ($post) {
            return $job->modelClass === TraitTestPost::class
                && $job->modelId === $post->id;
        });
    });

    it('does not dispatch analysis job when non-content field changes', function () {
        $post = TraitTestPost::create([
            'title' => 'Test Post',
            'slug' => 'test-post',
            'content' => 'Test content',
        ...
        $post->update(['featured_image' => '/images/new-image.jpg']);

        // Should not dispatch since featured_image is not in content fields
        Queue::assertNotPushed(AnalyzeContentJob::class);
    });
    ...
    it('respects auto analyze feature toggle', function () {
        config(['seo.features.auto_analyze' => false]);

        Queue::fake();

        $post = TraitTestPost::create([
            'title' => 'No Auto Analyze',
            'slug' => 'no-auto-analyze',
            'content' => 'Test content',
        ]);
