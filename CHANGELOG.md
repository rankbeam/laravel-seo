...
- Remove obsolete reflection-access calls from test helpers, avoiding PHP 8.5 deprecations.

## [3.20.1] - 2026-09-10

### Fixed

- Sitemap model inclusion and optional image/hreflang extensions reuse the same resolved metadata for each URL build. With resolver caching disabled this removes the second metadata lookup for included models with extensions. The reuse ends after each URL, including failure paths, so later generations and locale changes resolve fresh data. Custom Spatie URL tags keep their existing behavior.
...
- **The OG card's date was English on every card.** `OgImageGenerator::formatDate()` localised the byline date only for a Carbon instance, and the resolver hands the generator a `DateTimeImmutable`, so the article card of a page resolved in Italian still read "Sep 8, 2026". With ext-intl the date is now ICU's medium form for the page locale — "8 set 2026", "08.09.2026", "2026/09/08", "8 de set. de 2026" — so the order and the month are the locale's, not English with a translated month; without ext-intl a plain `DateTimeInterface` is wrapped in Carbon and gets the translated month it was always meant to get. Found dogfooding the Italian edition of blog.rankbeam.dev.
...
- **`SEODefaultsRepository::clearCache()` with no arguments now clears the cache store.** It emptied the per-request memo and bumped the memo version, then stopped — the comment said the store "will naturally expire" — so every persistent entry kept serving stale defaults until its TTL ran out. A full clear was not a clear. It now walks every scope with rows in `seo_defaults` plus every scope recorded as cached (a scope whose rows were deleted since is only in that second list, and is forgotten too) and forgets each one through the per-scope path 3.16.0 fixed, which knows every locale a scope was cached under. An entry written by a release before 3.16 is recorded nowhere in the store, so the full clear also forgets every scope/locale pair that has a row of its own; a pre-3.16 entry for a locale that only fell back to English cannot be named and expires within its TTL. The scope record is rewritten on every cache write, like the locale tracker.
- **The scope and locale trackers now outlive the entries they clear by a margin.** Both are written inside the `remember()` callback, before the entry itself, and used the entry's own TTL, so a tracker could expire moments before its last entry. They now carry a 60-second buffer.
## [3.16.0] - 2026-09-07
...
- **Per-script title and description budgets (`Rankbeam\Seo\I18n\LengthPolicy`).** The bare 60/160 constants are now the Latin row of a policy keyed by script (`latin`, `cyrillic`, `greek`, `cjk`, `thai`, `arabic`, `hebrew`, `devanagari`, with `default`), configured under `seo.length_policy`; CJK gets 30/80 built in (a full-width glyph is about twice as wide, so a 45-character Japanese title is already cut on Google). The editor warnings (`SEOWarningEvaluator`, which gained an optional `$locale` argument on `evaluateTitle()` / `evaluateDescription()`), the free audit and the computed-description truncation all read the policy, and lengths are counted in **graphemes** (a Thai syllable or an emoji with a modifier is one character). The `TITLE_MAX_LENGTH` / `DESCRIPTION_MAX_LENGTH` constants stay as the Latin defaults. Audit length findings carry the detected `script` in their context.
- **Grapheme-safe, script-aware truncation (`Rankbeam\Seo\I18n\Truncator`).** Computed descriptions in Han, Kana or Thai are cut at the last sentence or clause mark (。！？、，…) inside the limit instead of a non-existent word boundary, Korean at a space, and no cut can land inside a combining sequence. `seo.computed.description_max_length` stays a Latin budget and is scaled per script (a CJK description gets half). Latin output is byte-identical.
- **Locale-aware casing (`Rankbeam\Seo\I18n\CaseFolder`).** `lower()`, `fold()`, `equals()`, `contains()`, `containsWord()` handle the Turkish/Azeri dotted and dotless i, the Greek final sigma and the German sharp s, NFC-normalise when ext-intl is present, and use Unicode word boundaries. The brand-aware title-suffix skip (`seo.title_suffix_skip_when_contains`) now matches through it, so `İstanbul` matches `istanbul` and an accented brand gets a real word boundary.
- **hreflang hardening (`Rankbeam\Seo\I18n\Hreflang` + `seo.hreflang`).** Codes are validated against the ISO 639-1 / 15924 / 3166-1 (+ UN M.49) lists and parsed into subtags; a model's alternates go through three policies before they reach the `<link>` tags, the sitemap, `llms.txt` and the audit — one list, seen identically everywhere: `normalize` (**on by default**: `it_IT` → `it-IT`, `zh_hans_cn` → `zh-Hans-CN`; an underscore is not valid in hreflang, so this is a fix for anyone who handed over a Laravel locale), `include_self` (off: append the page's own locale + canonical when the list omits it — Google requires the self-reference) and `x_default` (null: the language whose alternate is duplicated as `x-default`). An empty list stays empty. Three core-only audit codes: `hreflang_invalid_code` (warning), `hreflang_duplicate_code` (notice), `hreflang_missing_self` (warning), translated in all ten languages.
- **`inLanguage` on the schema graph.** The WebPage node carries the page's resolved locale in BCP 47 form, `ArticleSchema::fromModel()` takes it from the stored `seo_meta` locale (also `setInLanguage()`), and the WebSite node lists `schema.website.inLanguage` (one code or a list). `seo.schema.in_language => false` restores the previous output.
- **Regional search engines in the crawler catalog.** Yandex, Baidu, Naver (`Yeti`), Seznam, Sogou, 360, Cốc Cốc and DuckDuckGo are catalogued with the new purpose `search_engine` (allowed by default) and take part in `seo.ai_crawlers.policy` / `overrides` and the `list => 'all'` output, so a site can keep a crawler it does not serve off its bandwidth with one override. `AiCrawlerRegistry::all()` and `match()` stay AI-only unless asked (`searchEngines()`, `all(true)`, `match($ua, true)`); `get()` and `byPurpose()` see both. The default `blocked`-mode robots.txt is byte-identical.
- **Search-engine site verification (`seo.verification`).** `google-site-verification`, `msvalidate.01`, `yandex-verification`, `baidu-site-verification`, `naver-site-verification`, `seznam-wmt`, `p:domain_verify` and `facebook-domain-verification` render as meta tags on every page for the keys you set (a value may be a list); nothing is emitted otherwise. Present in `render()`, `toArray()` and `toInertiaHead()` (head-keyed).
- **OG images in every script.** The three templates take a per-script fallback `font-family` stack (`seo.og_image.font_stack`, built-in Noto list) with the page language's CJK family moved to the front (Han unification), a BCP 47 `lang` attribute, and the stack is part of the cache key. `seo:og-images` probes the host with fontconfig (`FontProbe`) and warns once per script with the `apt-get` line when no installed font covers a title it is about to render; silent where the host cannot say. The live Browsershot smoke gained one glyph fixture per script (ja, zh-Hans, zh-Hant, ko, el, ru, tr, th, ar, he, hi) that renders the title and a same-length tofu control and fails when the PNGs are identical.
- **IDN-tolerant canonical validation (`Rankbeam\Seo\I18n\Url`).** The audit's `invalid_canonical` check accepts an IDN host and a Unicode or percent-encoded path (PHP's `FILTER_VALIDATE_URL` rejected both) and now insists on an absolute `http(s)` URL with a syntactically valid host.
- **`llms.txt` alternates (`seo.llms_txt.alternates`, off).** A page that exists in other languages ends its bullet with `Also in: [it](…), [de](…)` from the policy-applied alternates, minus `x-default` and itself.
- **`Rankbeam\Seo\I18n\Script`** — the dominant-script detector and grapheme counter behind all of the above, public for your own code.

### Changed

- hreflang codes returned by `getSEOAlternates()` are normalised to BCP 47 by default (see above); set `seo.hreflang.normalize => false` to keep them verbatim.
- The WebPage / Article JSON-LD gains an `inLanguage` key (see above); `seo.schema.in_language => false` to opt out.
- `AiCrawlerRegistry::directives()` includes the regional search engines; in the default `blocked` mode this changes nothing, in `all` mode eight explicit allow directives appear.
