...
`send_header` (on by default within the guard) also sends
`X-Robots-Tag: noindex,nofollow` on every response routed through the app, so
PDFs, feeds and images — which carry no `<meta robots>` — are held back too; the
middleware is registered only when the guard is enabled. Strongly recommended —
see the full [Indexing guard guide](/guide/indexing-guard).
...
`getUrlForSEO()`) has its query string stripped by default — tracking, filter and
sort params all create duplicate-content canonical targets for the same page.
List keys in `query_whitelist` and they are **kept** in derived canonicals, in
this order, while every other param is still stripped. The usual case is `page`
for paginated archives (`/blog?page=2` is genuinely not `/blog`).

An **explicitly set** canonical (admin-entered, or from a higher precedence
layer) is always emitted verbatim, query string and all — the whitelist governs
only the derived fallback. Default `[]` preserves the strip-everything behaviour.
